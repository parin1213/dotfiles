...
- **擬人化演出の副詞**：「黙って」「静かに」「そっと」「ひっそり（と）」「密かに」「淡々と」「粛々と」を、処理・失敗・システムの挙動の描写に付けない（英語 LLM の "silently fails" 直訳系。指摘 2026-08-05「AI語なので今後マジ辞めてくれ」）。事実をそのまま書くか、本人が使う「勝手に」を使う。悪い例：「変換処理が黙って元へ戻す」。良い例：「変換処理が勝手に元へ戻す」「変換処理が元へ戻す」
