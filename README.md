...
エージェントスキル（`~/.claude/skills/` 等）は**出所で 4 型に分けて管理**する。宣言は `skills/manifest.toml`、復元は `chezmoi apply`（`run_onchange_after_skills-setup` が `skills/setup.sh` / `setup.ps1` を実行）。
